...
const ChiTietBienBanBanGiao = () => {
  ...
  const handleSubmit = async (status) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Biên bản đã được cập nhật thành công.');
        navigate('/danh-sach-bien-ban-ban-giao');
      } else {
        throw new Error('Lỗi khi cập nhật biên bản');
      }
    } catch (error) {
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="d-flex justify-content-end mt-4">
        <button className="btn btn-warning mx-2" onClick={() => handleSubmit('Đã gửi duyệt')} style={{ minWidth: '100px' }} disabled={!isEditable}>
          Gửi
        </button>
        <button className="btn btn-primary mx-2" onClick={() => handleSubmit('Tạo mới')} style={{ minWidth: '100px' }} disabled={!isEditable}>
          Lưu
        </button>
        <button className="btn btn-secondary mx-2" onClick={() => navigate(-1)} style={{ minWidth: '100px' }}>
          Đóng
        </button>
      </div>
    </div>
...
export default ChiTietBienBanBanGiao;
