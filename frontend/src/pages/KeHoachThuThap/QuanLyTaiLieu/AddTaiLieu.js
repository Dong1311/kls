...
const AddTaiLieu = () => {
  ...
  const [taiLieu, setTaiLieu] = useState({
    tenTaiLieu: '',
    khungBienMuc: 'Văn bản',
    maDinhDanhVanBan: '',
    sttTrongHoSo: '',
    tenLoaiVanBan: 'Nghị quyết',
    soVanBan: '',
    kyHieuTaiLieu: '',
    ngayThangNamVB: '',
  ...
  const handleSave = () => {
    if (!taiLieu.hoSoId) {
      setUploadStatus('Vui lòng chọn mã hồ sơ trước khi lưu.')
      return
    }
    if (!taiLieu.ngayThangNamVB || isNaN(new Date(taiLieu.ngayThangNamVB).getTime())) {
      setUploadStatus('Ngày tháng năm VB không hợp lệ. Vui lòng chọn lại.')
      return
    }
    if (!taiLieu.link) {
    ...
    fetch(`/api/ho-so/${taiLieu.hoSoId}/tai-lieu`, {
      // Thêm tài liệu vào hồ sơ đã chọn
      method: 'POST',
      headers: {
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="row g-3">
        ...
        <SelectInput
          label="Khung biên mục"
          name="khungBienMuc"
          value={taiLieu.khungBienMuc}
          onChange={handleChange}
          options={['Văn bản', 'Tài liệu phim ảnh', 'Tài liệu phim âm thanh']}
        />
        <TextInput
          label="Trích yếu nội dung"
          name="trichYeuNoiDung"
          value={taiLieu.trichYeuNoiDung}
          onChange={handleChange}
        />
        <HoSoSelectInput
          label="Tên hồ sơ"
          name="hoSoId"
          value={taiLieu.hoSoId}
          onChange={handleChange}
          options={hoSoList.map((hoSo) => ({ value: hoSo.id, label: hoSo.tieuDeHoSo }))}
        />
        <SelectInput
        ...
        <TextInput
          ...
          name="maDinhDanhVanBan"
          value={taiLieu.maDinhDanhVanBan}
          onChange={handleChange}
        />
        ...
        <TextInput
          ...
          name="kyHieuThongTin"
          value={taiLieu.kyHieuThongTin}
          onChange={handleChange}
        />
        <TextInput
          label="Ký hiệu tài liệu"
          name="kyHieuTaiLieu"
          value={taiLieu.kyHieuTaiLieu}
          onChange={handleChange}
        />
        ...
        <SelectInput
          ...
          onChange={handleChange}
          options={['Bình thường', 'Bị mốc nhẹ', 'Bị hư hỏng']}
        />
        <SelectInput
