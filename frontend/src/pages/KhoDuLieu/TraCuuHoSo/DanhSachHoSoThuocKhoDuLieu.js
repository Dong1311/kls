...
const DanhSachHoSoThuocKhoDuLieu = () => {
  ...
  const fetchHoSos = () => {
    ...
    fetch(`/api/ho-so?${query}`)
      ...
      .then((data) => {
        const filteredData = data.filter((hoSo) => ['Đã nhận lưu kho', 'Chờ QĐTH'].includes(hoSo.trangThai))
        ...
        <tbody>
          {hoSoList.map((hoSo, index) => (
            <tr key={hoSo.id}>
              ...
              <td>
                <span style={{ ...getTrangThaiStyle(hoSo.trangThai), minWidth: '120px' }}>{hoSo.trangThai}</span>
              </td>
            </tr>
          ))}
        </tbody>
