...
const ChiTietHoSoDaTrinhDuyetLuuKho = () => {
  const { id } = useParams()
  ...
  const handleTuChoi = () => {
    const updatedHoSo = { ...hoSo, trangThai: 'Từ chối lưu kho' }
    fetch(`/api/ho-so/${id}`, {
      method: 'PUT',
      ...
      .then((data) => {
        setHoSo(data)
        alert('Hồ sơ đã được từ chối với trạng thái "Từ chối lưu kho".')
        navigate(-1)
      })
      .catch((error) => {
  ...
  const handleDuyet = () => {
    const updatedHoSo = { ...hoSo, trangThai: 'Đã nhận lưu kho' }
    fetch(`/api/ho-so/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(updatedHoSo),
      ...
      .then((data) => {
        setHoSo(data)
        alert('Hồ sơ đã được duyệt với trạng thái "Đã nhận lưu kho".')
        navigate(-1)
      })
      .catch((error) => {
        console.error('Lỗi khi nhận hồ sơ:', error)
        setError('Lỗi khi nhận hồ sơ')
      })
  }
      ...
      <div className="row g-3 mb-4">
        ...
        <div className="col-md-6 d-flex align-items-center mb-3">
          ...
          <input
            ...
            name="ngayBatDau"
            value={hoSo.ngayBatDau ? new Date(hoSo.ngayBatDau).toISOString().split('T')[0] : ''}
            onChange={handleInputChange}
            disabled={hoSo.trangThai === 'Đã nhận lưu kho' || hoSo.trangThai === 'Đã trình duyệt lưu kho'}
          />
        </div>
        <div className="col-md-6 d-flex align-items-center mb-3">
          ...
          <input
            ...
            className="form-control"
            name="ngayKetThuc"
            value={hoSo.ngayKetThuc ? new Date(hoSo.ngayKetThuc).toISOString().split('T')[0] : ''}
            onChange={handleInputChange}
            disabled={hoSo.trangThai === 'Đã nhận lưu kho' || hoSo.trangThai === 'Đã trình duyệt lưu kho'}
          />
        </div>
...
export default ChiTietHoSoDaTrinhDuyetLuuKho
